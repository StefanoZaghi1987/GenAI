...
class PRAnalyzer:
    def __init__(self):
        ...
        mcp_app = self.mcp.http_app(path='/mcp')

        # Pass lifespan to FastAPI
        self.app = FastAPI(title="Github PR Analysis API", lifespan=mcp_app.lifespan)

        # Register FastAPI endpoints
        self._register_endpoints()

        # Mount the MCP server
        self.app.mount("/github_pr_analysis", mcp_app)
    ...
    def run(self):
        ...
        try:
            ...
            uvicorn.run(self.app, host="0.0.0.0", port=8070)
        except Exception as e:
